...
## Деплой и обновление проектов

### Автоматический деплой (рекомендуется)

**С локальной машины** (требуется SSH-ключ в ssh-agent):
```bash
# Добавить ключ в агент (один раз за сессию)
eval "$(ssh-agent -s)"
...
2. `docker compose build` — пересобирает образы (для Docker-проектов)
3. `docker compose up -d` — перезапускает контейнеры
4. Перезагружает конфиг Caddy (без даунтайма)
5. Чистит старые Docker-образы

Статические сайты (english_training, math_training, russkij-yazyk) обновляются только через `git pull` — Caddy раздаёт файлы напрямую из примонтированных директорий.

### Первый деплой нового проекта

Если директория проекта не существует на сервере, скрипт автоматически клонирует репозиторий с GitHub.
