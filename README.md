...
**Автооткат.** Если новые контейнеры не поднялись (упали или не прошли healthcheck за
`DEPLOY_WAIT_TIMEOUT`, по умолчанию 120 с), `deploy.sh` возвращает сервисам образы, на которых
они работали до деплоя, а сам деплой считает неудачным — CI краснеет. Откатываются только образы:
compose-файл остаётся новым, миграции базы не отменяются. Прежние образы получают тег `rollback`.

Ручной откат — перезапустить job `deploy` у предыдущего успешного запуска в GitHub Actions
